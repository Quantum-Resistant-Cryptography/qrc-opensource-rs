...
use crate::qsc::{
	{
		digest::sha3::qsc_shake256_compute,
		tools::memutils::{
			qsc_memutils_copy,
			qsc_memutils_clear,
		},
		tools::intutils::qsc_intutils_min,
...
pub fn bgf_mul(out: &mut [Gf], in0: &[Gf], in1: &[Gf]) {
	...
	let byte_slice_one = cast_slice_mut::<Gf, u8>(out);
	let byte_slice_two = cast_slice::<Gf, u8>(prod);
	qsc_memutils_copy(byte_slice_one, byte_slice_two, MCELIECE_SYS_T * size_of::<Gf>());
}
...
pub fn synd(out: &mut [Gf], f: &[Gf], l: &[Gf], r: &[u8]) {
	...
	let byte_slice = cast_slice_mut::<Gf, u8>(out);
...
pub fn transpose_64x64(out: &mut [u64], int: &[u64]) {
	...
	let byte_slice_one = cast_slice_mut::<u64, u8>(out);
	let byte_slice_two = cast_slice::<u64, u8>(int);

	qsc_memutils_copy(byte_slice_one, byte_slice_two, 64 * size_of::<u64>());
...
pub fn bm(out: &mut [Gf], s: &[Gf]) {
	...
	for n in 0..(2 * MCELIECE_SYS_T) {
		...
		for i in 0..=qsc_intutils_min(n, MCELIECE_SYS_T) {
			d ^= gf_mul(c[i], s[n - i]);
		}
		...
		let byte_slice_one = cast_slice_mut::<Gf, u8>(t);
		let byte_slice_two = cast_slice::<Gf, u8>(c);
...
pub fn cbrecursion(out: &mut [u8], mut pos: i64, step: i64, pi: &[u16], w: i64, n: i64, temp: &mut [i32]) {
	...
	let qo = &mut temp[((n + n / 4) as usize)..].to_owned();
	let q = cast_slice_mut::<i32, i16>(qo);
	...
	let byte_slice_one = cast_slice::<i16, u16>(q);
	let byte_slice_two = cast_slice::<i16, u16>(&q[(n as usize / 2)..]);
	cbrecursion(out, pos, step * 2, byte_slice_one, w - 1, n / 2, temp);
	cbrecursion(out, pos + step, step * 2, byte_slice_two, w - 1, n / 2, temp);
}
...
pub fn controlbits_from_permutation(out: &mut [u8], pi: &[i16], w: i64, n: i64) {
	...
	loop {
		qsc_memutils_clear(out);
		let byte_slice = cast_slice::<i16, u16>(pi);
		cbrecursion(out, 0, 1, byte_slice, w, n, temp);
	...
	let byte_slice_one = cast_slice_mut::<i16, u8>(pi_test);
	let byte_slice_two = cast_slice_mut::<i32, u8>(temp);
	qsc_memutils_clear(byte_slice_one);
	qsc_memutils_clear(byte_slice_two);
}
...
pub fn decrypt(e: &mut [u8], mut sk: &[u8], c: &[u8]) -> i32 {
	...
	qsc_memutils_clear(&mut r[MCELIECE_SYND_BYTES..]);
...
pub fn gen_e(secrand_state: &mut QscSecrandState, e: &mut [u8], rng_generate: fn(&mut QscSecrandState, &mut [u8], usize) -> bool) {
	...
	if QSC_MCELIECE_S5N8192T128 {
		let brnd = &mut [0u8; MCELIECE_SYS_T * size_of::<u16>()];

		loop {
			rng_generate(secrand_state, brnd, MCELIECE_SYS_T * size_of::<u16>());
		...
		loop {
			rng_generate(secrand_state, brnd, MCELIECE_SYS_T * 2 * size_of::<u16>());		
...
pub fn pk_gen(pk: &mut [u8], mut sk: &[u8], perm: &[u32], pi: &mut [i16]) -> i32 {
	...
	if res != -2 {
		...
		if QSC_MCELIECE_S5N6960T119 {
			...
			for i in 0..MCELIECE_PK_NROWS {
				qsc_memutils_copy(&mut pk[(i * MCELIECE_PK_ROW_BYTES)..], &mat[i][(MCELIECE_PK_NROWS / 8)..], MCELIECE_PK_ROW_BYTES);
			}
		}
	}
	res = 0;
...
pub fn qsc_mceliece_ref_encapsulate(secrand_state: &mut QscSecrandState, c: &mut [u8], key: &mut [u8], pk: &[u8], rng_generate: fn(&mut QscSecrandState, &mut [u8], usize) -> bool) -> i32 {
	...
	one_ec[0] = 1;
	two_e[0] = 2;
	encrypt(secrand_state, c, pk, e, rng_generate);
	qsc_memutils_copy(&mut two_e[1..], e, MCELIECE_SYS_N / 8);

	qsc_shake256_compute(&mut c[MCELIECE_SYND_BYTES..], MCELIECE_SHAREDSECRET_SIZE, two_e, 1 + MCELIECE_SYS_N / 8);
	qsc_memutils_copy(&mut one_ec[1..], e, MCELIECE_SYS_N / 8);
	qsc_memutils_copy(&mut one_ec[1 + MCELIECE_SYS_N / 8..], c, MCELIECE_SYND_BYTES + 32);
	qsc_shake256_compute(key, MCELIECE_SHAREDSECRET_SIZE, one_ec, 1 + MCELIECE_SYS_N / 8 + (MCELIECE_SYND_BYTES + 32));
...
pub fn qsc_mceliece_ref_decapsulate(key: &mut [u8], c: &[u8], sk: &[u8]) -> i32 {
	...
	qsc_shake256_compute(conf, MCELIECE_SHAREDSECRET_SIZE, two_e, 1 + MCELIECE_SYS_N / 8);
	...
	qsc_shake256_compute(key, MCELIECE_SHAREDSECRET_SIZE, preimage, 1 + MCELIECE_SYS_N / 8 + (MCELIECE_SYND_BYTES + 32));
...
pub fn qsc_mceliece_ref_generate_keypair(secrand_state: &mut QscSecrandState, pk: &mut [u8], sk: &mut [u8], rng_generate: fn(&mut QscSecrandState, &mut [u8], usize) -> bool) -> i32 {
	...
	rng_generate(secrand_state, &mut seed[1..], 32);
	...
	loop {	
		...
		qsc_shake256_compute(r, rlen, seed, 33);
		qsc_memutils_copy(skp, &seed[1..], 32);
		skp = &mut skp[32 + 8..];

		qsc_memutils_copy(&mut seed[1..], &r[(rlen - 32)..], 32);
