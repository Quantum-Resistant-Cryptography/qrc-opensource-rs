...
*/
pub(crate) fn qsc_memutils_clear(output: &mut [u8]) {
	output.fill(0);
}

pub(crate) fn qsc_memutils_clear_string(output: &mut String) {
    swap(output, &mut String::with_capacity(qsc_stringutils_string_size(output)));
}
 
pub(crate) fn qsc_memutils_copy(output: &mut [u8], input: &[u8], length: usize) {
	if length != 0 {
		for i in 0..length {
			output[i] = input[i];
		}
...
pub(crate) fn qsc_memutils_xor(output: &mut [u8], input: &[u8], length: usize) {
	if length != 0 {
