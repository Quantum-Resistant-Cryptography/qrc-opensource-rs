...
pub fn qrc_aes_hba256_transform(state: &mut QrcAesHba256State, output: &mut [u8], input: &[u8], length: usize) -> bool {
	...
	if state.encrypt {
		...
		if qrc_intutils_verify(code, &input[length..], QRC_HBA256_MAC_LENGTH) == 0 {
			...
			/* Fake decrypt to ensure timing */
			let mut fake_state = state.cstate.clone();
			let fake_output_len = output.len();
			let mut fake_output = vec![0u8; fake_output_len];
			qrc_intutils_copy8(&mut fake_output, output, fake_output_len);
			qrc_aes_ctrle_transform(&mut fake_state, &mut fake_output, input, length);
			qrc_aes_dispose(&mut fake_state);
			qrc_intutils_clear8(&mut fake_output, fake_output_len);
		}
	}
...
fn gf_mul(mut a: u8, mut b: u8) -> u8 {
    let mut res: u16 = 0;
    for _ in 0..8 {
        let mask = 0u16.wrapping_sub((b & 1) as u16);
        res ^= (a as u16) & mask;

        let hi = (a & 0x80) != 0;
        a = a << 1;
        let red = 0u8.wrapping_sub(hi as u8) & 0x1B;
        a ^= red;
...
fn rotl8(x: u8, r: u32) -> u8 {
	return x.rotate_left(r);
}
...
fn inverse_affine_transform(s: u8) -> u8 {
    return rotl8(s, 1) ^ rotl8(s, 3) ^ rotl8(s, 6) ^ 0x05;
}
...
fn inv_sbox_u8(s: u8) -> u8 {
    let b = inverse_affine_transform(s);
   	return gf_inv(b);
}
...
fn aes_sub_bytes(state: &mut [u8]) {
	for i in 0..QRC_AES_BLOCK_SIZE {
		state[i] = sbox_u8(state[i]);
	}
}

fn aes_substitution(rot: u32) -> u32 {
    let b0 = sbox_u8((rot & 0xFF) as u8) as u32;
    let b1 = sbox_u8(((rot >> 8) & 0xFF) as u8) as u32;
    let b2 = sbox_u8(((rot >> 16) & 0xFF) as u8) as u32;
    let b3 = sbox_u8(((rot >> 24) & 0xFF) as u8) as u32;
