...
pub fn sphincsplus_ref_generate_seed_keypair(pk: &mut [u8], sk: &mut [u8], seed: &[u8]) -> i32 {
    ...
    qrc_intutils_copy8(sk, seed, SPHINCSPLUS_CRYPTO_SEEDBYTES);
    qrc_intutils_copy8(pk, &sk[2 * SPX_N..], SPX_N);
...
*/
pub fn sphincsplus_ref_generate_keypair(asymmetric_state: &mut AsymmetricRandState, pk: &mut [u8], sk: &mut [u8], rng_generate: fn(&mut AsymmetricRandState, &mut [u8], usize) -> bool) {
    ...
    rng_generate(asymmetric_state, seed, SPHINCSPLUS_CRYPTO_SEEDBYTES);
...
* \param rng_generate: A pointer to the random generator function
*/
pub fn sphincsplus_ref_sign_signature(asymmetric_state: &mut AsymmetricRandState, mut sig: &mut [u8], siglen: &mut usize, m: &[u8], mlen: usize, sk: &[u8], rng_generate: fn(&mut AsymmetricRandState, &mut [u8], usize) -> bool) {
    ...
    rng_generate(asymmetric_state, optrand, SPX_N);
...
pub fn sphincsplus_ref_sign_verify(mut sig: &[u8], siglen: usize, m: &[u8], mlen: usize, pk: &[u8]) -> bool {
    ...
    if siglen == SPX_BYTES {
        ...
        res = qrc_intutils_verify(root, pub_root, SPX_N) == 0;
    }
...
* \param rng_generate: A pointer to the random generator function
*/
pub fn sphincsplus_ref_sign(asymmetric_state: &mut AsymmetricRandState, sm: &mut [u8], smlen: &mut usize, m: &[u8], mlen: usize, sk: &[u8], rng_generate: fn(&mut AsymmetricRandState, &mut [u8], usize) -> bool) {
    /* Returns an array containing the signature followed by the message */

    let mut siglen = 0;

    sphincsplus_ref_sign_signature(asymmetric_state, sm, &mut siglen, m, mlen, sk, rng_generate);
    qrc_intutils_copy8(&mut sm[SPX_BYTES..], m, mlen);
    *smlen = siglen + mlen;
}
...
const SPX_N: usize = if QRC_SPHINCSPLUS_S3S192SHAKERF {
    24
} else if QRC_SPHINCSPLUS_S3S192SHAKERS {
    24
...
const SPX_FULL_HEIGHT: usize = if QRC_SPHINCSPLUS_S3S192SHAKERF {
    66
} else if QRC_SPHINCSPLUS_S3S192SHAKERS {
    63
} else if QRC_SPHINCSPLUS_S5S256SHAKERF {
...
const SPX_D: usize = if QRC_SPHINCSPLUS_S3S192SHAKERF {
    22
} else if QRC_SPHINCSPLUS_S3S192SHAKERS {
    7
} else if QRC_SPHINCSPLUS_S5S256SHAKERF {
...
const SPX_FORS_HEIGHT: usize = if QRC_SPHINCSPLUS_S3S192SHAKERF {
    8
} else if QRC_SPHINCSPLUS_S3S192SHAKERS {
    14
} else if QRC_SPHINCSPLUS_S5S256SHAKERF {
...
const SPX_FORS_TREES: usize = if QRC_SPHINCSPLUS_S3S192SHAKERF {
    33
} else if QRC_SPHINCSPLUS_S3S192SHAKERS {
    17
} else if QRC_SPHINCSPLUS_S5S256SHAKERF {
...
const SPX_WOTS_W: usize = if QRC_SPHINCSPLUS_S3S192SHAKERF {
    16
} else if QRC_SPHINCSPLUS_S3S192SHAKERS {
    16
} else if QRC_SPHINCSPLUS_S5S256SHAKERF {
...
fn sphincsplus_set_layer_addr(addr: &mut [u32; 8], layer: u32) {
    ...
    let mut addr_slice = qrc_intutils_transform_32to8(addr);
    addr_slice[SPX_OFFSET_LAYER] = layer as u8;
...
fn sphincsplus_set_tree_addr(addr: &mut [u32; 8], tree: u64) {
    ...
    if SPX_TREE_HEIGHT * (SPX_D - 1) <= 64 {
        let mut addr_slice = qrc_intutils_transform_32to8(addr);
        qrc_intutils_be64to8(&mut addr_slice[SPX_OFFSET_TREE..], tree);
        qrc_intutils_copy32(addr, &qrc_intutils_transform_8to32(&addr_slice), 8);
        qrc_intutils_clear8all(&mut addr_slice);
    }
}
...
fn sphincsplus_set_type(addr: &mut [u32; 8], type_var: u32) {
    ...
    let mut addr_slice = qrc_intutils_transform_32to8(addr);
    addr_slice[SPX_OFFSET_TYPE] = type_var as u8;
    qrc_intutils_copy32(addr, &qrc_intutils_transform_8to32(&addr_slice), 8);
    qrc_intutils_clear8all(&mut addr_slice);
}
...
fn sphincsplus_copy_subtree_addr(out: &mut [u32; 8], int: &mut [u32; 8]) {
    ...
    qrc_intutils_copy32(out, int, (SPX_OFFSET_TREE + 8)/size_of::<u32>());
}
...
fn sphincsplus_copy_keypair_addr(out: &mut [u32; 8], int: [u32; 8]) {
    ...
    let mut out_slice = qrc_intutils_transform_32to8(out);
...
fn sphincsplus_set_chain_addr(addr: &mut [u32; 8], chain: u32) {
    ...
    let mut addr_slice = qrc_intutils_transform_32to8(addr);
    addr_slice[SPX_OFFSET_CHAIN_ADDR] = chain as u8;
    qrc_intutils_copy32(addr, &qrc_intutils_transform_8to32(&addr_slice), 8);
    qrc_intutils_clear8all(&mut addr_slice);
}
...
fn sphincsplus_set_hash_addr(addr: &mut [u32; 8], hash: u32) {
    ...
    let mut addr_slice = qrc_intutils_transform_32to8(addr);
    addr_slice[SPX_OFFSET_HASH_ADDR] = hash as u8;
    qrc_intutils_copy32(addr, &qrc_intutils_transform_8to32(&addr_slice), 8);
    qrc_intutils_clear8all(&mut addr_slice);
}
...
fn sphincsplus_set_tree_height(addr: &mut [u32; 8], tree_height: u32) {
    ...
    let mut addr_slice = qrc_intutils_transform_32to8(addr);
    addr_slice[SPX_OFFSET_TREE_HGT] = tree_height as u8;
    qrc_intutils_copy32(addr, &qrc_intutils_transform_8to32(&addr_slice), 8);
    qrc_intutils_clear8all(&mut addr_slice);
}
...
fn sphincsplus_set_tree_index(addr: &mut [u32; 8], tree_index: u32) {
    ...
    let mut addr_slice = qrc_intutils_transform_32to8(addr);
    sphincsplus_u32_to_bytes(&mut addr_slice[SPX_OFFSET_TREE_INDEX..], tree_index);
    qrc_intutils_copy32(addr, &qrc_intutils_transform_8to32(&addr_slice), 8);
    qrc_intutils_clear8all(&mut addr_slice);
}
/*
...
fn sphincsplus_prf_addr(out: &mut [u8], key: &[u8], addr: &mut [u32; 8]) {
    ...
    qrc_intutils_copy8(buf, key, SPX_N);

    qrc_intutils_copy8(&mut buf[SPX_N..], &qrc_intutils_transform_32to8(addr), SPX_ADDR_BYTES);

    qrc_shake256_compute(out, SPX_N, buf, SPX_N + SPX_ADDR_BYTES);
}
...
fn sphincsplus_gen_message_random(r: &mut [u8], sk_prf: &[u8], optrand: &[u8], m: &[u8], mlen: u64) {
    ...
    qrc_keccak_incremental_absorb(kctx, QRC_KECCAK_256_RATE, sk_prf, SPX_N);
    qrc_keccak_incremental_absorb(kctx, QRC_KECCAK_256_RATE, optrand, SPX_N);
    qrc_keccak_incremental_absorb(kctx, QRC_KECCAK_256_RATE, m, mlen as usize);
    qrc_keccak_incremental_finalize(kctx, QRC_KECCAK_256_RATE, QRC_KECCAK_SHAKE_DOMAIN_ID);
    qrc_keccak_incremental_squeeze(kctx, QRC_KECCAK_256_RATE, r, SPX_N);
}
...
fn sphincsplus_hash_message(digest: &mut [u8], tree: &mut u64, leaf_idx: &mut u32, r: &[u8], pk: &[u8], m: &[u8], mlen: u64) {
    ...
    qrc_keccak_incremental_absorb(kctx, QRC_KECCAK_256_RATE, r, SPX_N);
    qrc_keccak_incremental_absorb(kctx, QRC_KECCAK_256_RATE, pk, SPX_PK_BYTES);
    qrc_keccak_incremental_absorb(kctx, QRC_KECCAK_256_RATE, m, mlen as usize);
    qrc_keccak_incremental_finalize(kctx, QRC_KECCAK_256_RATE, QRC_KECCAK_SHAKE_DOMAIN_ID);
    qrc_keccak_incremental_squeeze(kctx, QRC_KECCAK_256_RATE, buf, SPX_DGST_BYTES);

    let mut bufp: &[u8] = &buf.to_owned();

    qrc_intutils_copy8(digest, &bufp, SPX_FORS_MSG_BYTES);
...
fn sphincsplus_thash(out: &mut [u8], int: &[u8], inblocks: u32, pubseed: &[u8], addr: &mut [u32; 8]) {
    ...
    qrc_intutils_copy8(buf, pubseed, SPX_N);

    qrc_intutils_copy8(&mut buf[SPX_N..], &qrc_intutils_transform_32to8(addr), SPX_ADDR_BYTES);

    qrc_shake256_compute(bitmask, blklen, &buf, keylen);
...
fn sphincsplus_compute_root(root: &mut [u8], leaf: &[u8], mut leaf_idx: u32, mut idx_offset: u32, mut auth_path: &[u8], tree_height: u32, pubseed: &[u8], addr: &mut [u32; 8]) {
    ...
    if (leaf_idx & 1) != 0 {
        qrc_intutils_copy8(&mut buffer[SPX_N..], leaf, SPX_N);
        qrc_intutils_copy8(buffer, auth_path, SPX_N);
    } else {
        qrc_intutils_copy8(buffer, leaf, SPX_N);
        qrc_intutils_copy8(&mut buffer[SPX_N..], auth_path, SPX_N);
    }
...
fn sphincsplus_treehash(root: &mut [u8], auth_path: &mut [u8], sk_seed: &[u8], pubseed: &[u8], leaf_idx: u32, idx_offset: u32, tree_height: u32,
    ...
    for idx in 0..(1 << tree_height) as u32 {
        ...
        if (leaf_idx ^ 0x1) == idx {
            qrc_intutils_copy8(auth_path, &stack[(offset - 1) * SPX_N..], SPX_N);
        }
        ...
        loop {
            ...
            if ((leaf_idx >> heights[offset - 1]) ^ 0x1) == tree_idx {
                qrc_intutils_copy8(&mut auth_path[heights[offset - 1] as usize * SPX_N..], &stack[(offset - 1) * SPX_N..], SPX_N);
            }
        }
    }

    qrc_intutils_copy8(root, &stack, SPX_N);
    heights.fill(0);
    stack.fill(0);
}
...
fn sphincsplus_gen_chain(out: &mut [u8], int: &[u8], start: u32, steps: u32, pubseed: &[u8], addr: &mut [u32; 8]) {
    ...
    qrc_intutils_copy8(out, int, SPX_N);
